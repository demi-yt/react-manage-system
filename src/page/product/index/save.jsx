...
class ProductSave extends React.Component{
    ...
    // 上传图片成功
    onUploadSuccess(res) {
        let subImages = this.state.subIamges;
        subImages.push(res);
        this.setState({
            subImages : subImages
        });
    ...
    onImageDelete(e) {
        let index = parseInt(e.target.getAttribute('index')),
            subImages = this.state.subIamges;
    ...
    getSubImagesString() {
        return this.state.subIamges.map((image) => image.url).join(',');
    }
    // 表单提交
    onSubmit() {
        let product = {
        ...
        if(productCheckResult.status) {
            _product.saveProduct(product).then((res) => {
                _index.successTips(res);
                this.props.history.push('/product/index');
            },(errMsg) => {
                _index.errorTips(errMsg);
            })
        }else{
            _index.errorTips(productCheckResult.msg);
        }
    }
    render(){
        return (
